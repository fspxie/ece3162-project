...
class Informed_RRTStar(RRT):
    ...
    def RotationToWorldFrame(self, goal, start, c_min):
        '''
        ...
        E1 = np.subtract(goal, start) / c_min
    ...
    def informed_rrt_star(self, n):
        ...
        x_sol = list()
        c_min = np.linalg.norm(np.subtract(self.x_goal, self.x_init))

        x_center = np.add(self.x_init, self.x_goal) / 2
        C = self.RotationToWorldFrame(self.x_goal, self.x_init, c_min)
        r_goal = 50

        for i in range(n):
            if len(x_sol) == 0: c_best = None
            else: c_best = min([path_cost(self.trees[0].E, self.x_init, s) + segment_cost(s, self.x_goal) for s in x_sol])
            ...
            while x_new == None:
                if c_best != None:
                    ...
                    if not self.X.obstacle_free(x_rand): continue
                else:
                    x_rand = self.X.sample_free()
                
                x_nearest = self.get_nearest(0, x_rand)
                x_new = self.bound_point(steer(x_nearest, x_rand, self.q))

            L_near = self.get_nearby_vertices(0, self.x_init, x_new)

            connected = self.connect_shortest_valid(0, x_new, L_near)
            if not connected: continue

            self.rewire(0, x_new, L_near)

            if np.linalg.norm(np.subtract(self.x_goal, x_new)) < r_goal:
                x_sol.append(x_new)
